...
extension SyllableDetectorConfig
{
    ...
    private static func parseMapping(nm: String, withCount cnt: Int, from data: [String: String]) throws -> MappingFunction {
    ...
    init(fromTextFile path: String) throws {
        ...
        // get input mapping
        let inputMapping = try SyllableDetectorConfig.parseMapping("mapInputs", withCount: layers[0].inputs, from: data)
        let outputMapping = try SyllableDetectorConfig.parseMapping("mapOutputs", withCount: layers[layerCount - 1].outputs, from: data)
        
        // create network
        net = NeuralNet(layers: layers, inputMapping: inputMapping, outputMapping: outputMapping)
    }
}
